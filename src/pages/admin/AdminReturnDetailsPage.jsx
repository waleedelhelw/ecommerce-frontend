...
const AdminReturnDetailsPage = () => {
  ...
  const fetchReturn = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await adminReturnService.getReturnById(id);
  ...
  const handleApprove = async (data) => {
    try {
      ...
      await adminReturnService.approveReturn(id, data);
  ...
  const handleReject = async (data) => {
    try {
      setActionLoading(true);
      await adminReturnService.rejectReturn(id, data);
  ...
  const handleProcessRefund = async () => {
    try {
      setActionLoading(true);
      await adminReturnService.processRefund(id);
  ...
  return (
    <div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-orange-900 text-lg">
                  طلب مُصعَّد للإدارة - يحتاج تدخل!
                </p>
                <p className="text-sm text-orange-700">
                  البائع صعّد هذا الطلب لحل النزاع. راجع التفاصيل واتخذ القرار
                  المناسب.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-yellow-900">
                  طلب فى انتظار البائع
                </p>
                <p className="text-sm text-yellow-700">
                  البائع لم يرد بعد. يمكنك التدخل والموافقة/الرفض نيابة عنه.
                </p>
              </div>
            </div>
      ...
      {/* 📥 Received - Admin can process refund */}
      {returnRequest.status === 'Received' && (
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-indigo-900">
                  البائع استلم المنتج - يمكنك صرف المبلغ
                </p>
                <p className="text-sm text-indigo-700">
                  لو البائع تأخر فى الصرف، تقدر تتدخل وتصرف المبلغ للعميل.
                </p>
              </div>
            </div>
            <button
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-blue-700">
                وافق البائع على الطلب. العميل لازم يشحن المنتج
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold text-purple-900">
                العميل شحن المنتج - فى انتظار استلام البائع
              </p>
              <p className="text-sm text-purple-700">
                لو البائع رفض استلامه أو تأخر، يمكنك التدخل.
              </p>
            </div>
          </div>
        </div>
      ...
      {returnRequest.status === 'Cancelled' && (
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 mb-6">
          <p className="font-bold text-gray-900">
            🚫 تم إلغاء طلب الإرجاع من العميل
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div>
                ...
                {reasonInfo.fault && (
                  <p className="text-xs mt-2">
                    <span className="text-gray-500">المسؤول عن الخطأ: </span>
                    <span
                      className={`font-bold ${
                        reasonInfo.fault === 'Seller'
                          ? 'text-red-600'
                          : reasonInfo.fault === 'Customer'
                          ? 'text-orange-600'
                          : 'text-blue-600'
                      }`}
                    >
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border p-6">
            ...
            <div className="space-y-2 text-sm">
              <p className="flex items-center gap-2 text-gray-700">
                <FiHome size={14} />
                <Link
                  to={`/admin/sellers/${returnRequest.sellerId}`}
                  className="text-blue-600 hover:underline"
                >
                  {returnRequest.storeName || returnRequest.sellerName || '—'}
                </Link>
              </p>
              {returnRequest.sellerName && (
                <p className="flex items-center gap-2 text-gray-700">
                  <FiUser size={14} /> {returnRequest.sellerName}
                </p>
              )}
            </div>
          </div>
          ...
          {/* ⚡ Quick Actions (للـ Admin بس) */}
          {(canApproveOrReject || canProcessRefund) && (
                ...
                {canApproveOrReject && (
                  <>
                    <button
                      onClick={() => setShowApprove(true)}
                      disabled={actionLoading}
                      className="w-full bg-green-600 text-white px-3 py-2 rounded-lg hover:bg-green-700 disabled:opacity-50 text-sm font-medium flex items-center justify-center gap-1.5"
                    >
                    ...
                    <button
                      onClick={() => setShowReject(true)}
                      disabled={actionLoading}
                      className="w-full bg-red-600 text-white px-3 py-2 rounded-lg hover:bg-red-700 disabled:opacity-50 text-sm font-medium flex items-center justify-center gap-1.5"
                    >
                      <FiXCircle size={14} /> رفض
                    </button>
                  </>
                )}
                {canProcessRefund && (
                  <button
                    onClick={() => setShowProcessRefund(true)}
                    disabled={actionLoading}
                    className="w-full bg-emerald-600 text-white px-3 py-2 rounded-lg hover:bg-emerald-700 disabled:opacity-50 text-sm font-medium flex items-center justify-center gap-1.5"
                  >
                    <FiDollarSign size={14} /> صرف المبلغ
                  </button>
                )}
              </div>
            </div>
